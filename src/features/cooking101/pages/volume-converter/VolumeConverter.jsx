...
import ConverterButtons from "./volConvData";
...
const VOLUME_UNITS = [
  ...
  { label: "Ounces", value: "flOz" },
  { label: "Pints", value: "pt" },
];

export default function VolumeConverter() {
  const [input, setInput] = useState({ value: "0", unit: "cup" });
  const [output, setOutput] = useState({ value: 0, unit: "ml" });

  // Recalculate when unit dropdowns change
  useEffect(() => {
    setOutput((prevOutput) => ({
      ...prevOutput,
      value: volumeConversion(input.value, input.unit, prevOutput.unit),
    }));
